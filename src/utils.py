...
def prep_for_plot(img, rescale=True, resize=None):
    ...
    #plot_img = unnorm(img).squeeze(0).cpu()
    if rescale:
...
class UnsupervisedMetrics(Metric):
    ...
    def compute(self):
        ...
        metric_dict = {self.prefix + "mIoU": iou[~torch.isnan(iou)].mean().item(),
                       ...
                       self.prefix + "Mean_F1": f1.mean().item()}
        #print(metric_dict)
...
def flexible_collate(batch):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        if torch.utils.data.get_worker_info() is not None:
            ...
            '''if out.shape[0] // (320*320) == len(batch):
                out = out.view(len(batch),320,320)
            else:
                out = out.view(len(batch),3,320,320)'''
            #print(out.shape)
        try:
            #print(torch.stack(batch, 0, out=out))
