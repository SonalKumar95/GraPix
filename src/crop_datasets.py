...
class RandomCropComputer(Dataset):
    ...
    def __init__(self, cfg, dataset_name, img_set, crop_type, crop_ratio):
        ...
        if self.dataset_name == 'directory': 
            self.save_dir = join(cfg.pytorch_data_dir, "5{}".format(cfg.dir_dataset_name))
            self.img_dir = join(self.save_dir, "imgs", img_set)
            self.label_dir = join(self.save_dir, "labels", img_set)
        else:
            self.save_dir = join(cfg.pytorch_data_dir, "cropped", "{}_{}_crop_{}".format(dataset_name, crop_type, crop_ratio))
...
@hydra.main(config_path="configs", config_name="main.yml")
def my_app(cfg: DictConfig) -> None:
    print(OmegaConf.to_yaml(cfg))
    random.seed(0)
	
    #dataset_names = ["cityscapes", "directory"]
    #img_sets = ["train", "val"]
    #crop_types = ["five"]
    #crop_ratios = [.5]
